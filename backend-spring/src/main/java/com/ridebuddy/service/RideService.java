...
public class RideService {
    ...
    public java.util.List<BookingDtos.Response> bookings(UUID userId) { return bookings.findByUser_IdOrderByCreatedAtDesc(userId).stream().map(b -> new BookingDtos.Response(b.getId(), b.getRide().getId(), userId, b.getSeats(), b.getStatus())).toList(); }
    public java.util.Map<String,Object> bookingForRide(UUID userId, UUID rideId) { var booking = bookings.findByRide_IdAndUser_Id(rideId, userId).filter(b -> b.getStatus() == BookingStatus.CONFIRMED); var result = new java.util.HashMap<String,Object>(); result.put("alreadyBooked", booking.isPresent()); result.put("booking", booking.map(b -> new BookingDtos.Response(b.getId(), rideId, userId, b.getSeats(), b.getStatus())).orElse(null)); return result; }
